    ...
    row.forEach((symbol, j) => {
        switch (symbol) {
            ...
            case 'b':
                ...
                break
            //aggiungere qui altri casi (lasciare per ultimi i powerups e i pellets)
            case 'p':
                powerUps.push(
                    new PowerUp({
                        position: {
...
function animate() {
    ...
    for(let i = ghosts.length - 1; 0 <= i; i--) {
        ...
        if(Math.hypot(ghost.position.x - player.position.x, ghost.position.y - player.position.y) < ghost.radius + player.radius) {
            if(ghost.scared) {
                ...
                window.open('assets/you_lose.html', '_self')
            }
        }
    }
    ...
    if(pellets.length === 1) {
        cancelAnimationFrame(animationID)
        window.open('assets/you_win.html', '_self')
    }
